# -*- coding: utf-8 -*-
"""
Created on Wed Jul  5 08:17:11 2023

@author: hjammoul
Test flat SE with while value
Test duality, we're still validating erosion & dilation extension to color
"""
from pretreatment import normalize_without_ref, complemantarity 
from RGB_MorphoMath_tools import erosion_rgb, dilation_rgb ,Construct_SE_flat
...
Value = np.array((1.,1.,1.), dtype=np.float32) ##value in flat SE in RGB: white
Value = color.rgb2lab(Value, illuminant='D65') #SE flat value in lab: white
...
#######MAIN
###############################################################################################################"
SE_lab = Construct_SE_flat(Value,size_of_SE) #consruct the SE flat of color black
img_lab = normalize_without_ref(rgb_img) #convert RGB img to lab 
...
# Plot eroded_lab2
plt.subplot(2, 4, 6)
plt.imshow(eroded_value2.astype(np.int8))
plt.title("Erosion (O_inf: {}, O_sup: {})".format(O_inf_lab2, O_sup_lab2))
# Plot dilated_lab2
plt.subplot(2, 4, 7)
plt.imshow(dilated_value2.astype(np.int8))
